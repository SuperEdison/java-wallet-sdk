...
public final class TronRawTransaction implements RawTransaction {
    ...
    public TransactionType getType() {
        return switch (type) {
            case TRANSFER_CONTRACT -> TransactionType.TRON_TRANSFER;
            case TRIGGER_SMART_CONTRACT -> TransactionType.TRON_TRC20;
        };
    }
    ...
    public byte[] encode() {
        ...
        if (type == TronTransactionType.TRANSFER_CONTRACT) {
            contractContent = TronProtobuf.encodeTransferContract(ownerAddress, toAddress, amount);
            typeUrl = TronProtobuf.getTransferContractTypeUrl();
        } else {
            contractContent = TronProtobuf.encodeTriggerSmartContract(ownerAddress, toAddress, amount, data);
            typeUrl = TronProtobuf.getTriggerSmartContractTypeUrl();
        }

        // 2. 编码 Any (type_url + value)
        byte[] anyParameter = TronProtobuf.encodeAny(typeUrl, contractContent);

        // 3. 编码 Contract (type + parameter)
        byte[] contract = TronProtobuf.encodeContract(type.getValue(), anyParameter);
    ...
    @Override
    public byte[] hash() {
        return Sha256.hash(encode());
    }

    public TronTransactionType getTronType() {
        return type;
    }
