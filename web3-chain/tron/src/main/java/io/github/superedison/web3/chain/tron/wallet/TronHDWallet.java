...
/**
 * TRON HD 钱包实现，支持 BIP-32/39/44 派生。
 * 默认路径：m/44'/195'/0'/0/0
 */
public final class TronHDWallet extends TronWallet implements HDWallet {
    ...
    public TronHDWallet(List<String> mnemonic, String passphrase, String derivationPath,
    ...
    public HDWallet deriveChild(int index) {
        ...
        return new TronHDWallet(mnemonic, passphrase, childPath, childKey, getKeyHolder().canExportPrivateKey());
    }
    ...
    public HDWallet derivePath(String path) {
        ...
        try (Bip32.ExtendedKey masterKey = Bip32.masterKeyFromSeed(seed)) {
            ...
            return new TronHDWallet(mnemonic, passphrase, path, target, getKeyHolder().canExportPrivateKey());
        } finally {
    ...
    public List<Address> deriveAddresses(int startIndex, int count) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            try {
                ...
                addresses.add(TronAddress.fromPublicKey(publicKey));
            } finally {
                childKey.destroy();
            }
        }
